...
public class DescendingTraceProcessor extends AbstractTraceProcessor {
    ...
    private LevelState getLevelState(double levelGap,Inventory inventory) {
        ...
        if (tankSession.isOnFueling()) {
            ...
            //付油量超过100，但液位下降量没超过3
            if(blockCheck.getOutSum()+outSum>=100 && Math.abs(inventory.getLevel()-blockCheck.getFristInventory().getLevel())<3){
            ...
            else if(blockCheck.getOutSum()+outSum>=100 && Math.abs(inventory.getLevel()-blockCheck.getFristInventory().getLevel())>=3){
                ...
                if(AirUtils.hv(dbAlarmLists)) {
                    dbAlarmLists.stream().forEach(x -> {
                        ...
                        x.setClosed(true);
                        x.setSumOut(blockCheck.getOutSum());
