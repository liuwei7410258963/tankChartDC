...
public class DbInventoryServiceImpl extends ServiceImpl<DbInventoryDAO, DbInventory> implements DbInventoryService {
	 ...
	 */
	@Override
	public boolean save(DbInventory dbInventory) {
		try {
			String saveTableName = unionTableService.getAndCheckSubTable(dbInventory.getSubTableTime()
					, INVENTORY_TABLE_NAME, UnionTable.UNION_TYPE_INV);
			if (dbInventory.getId()==null){
				log.info("已经有罐存id:"+dbInventory);
