...
public class NozzleOutServiceImpl extends ServiceImpl<NozzleOutDao, BzNozzleOut> implements NozzleOutService {
	...
	public void processNozzleOutFromDit(BzNozzleOut bzNozzleOut) {
		...
		//处于异常波动中，预留字段设置为1
		if(AirUtils.hv(bzNozzleOut.getTankNo()) && errorMap.containsKey(bzNozzleOut.getTankNo()) && errorMap.get(bzNozzleOut.getTankNo())>=ERROR_NUM){
			bzNozzleOut.setReservedField(1);
		}
