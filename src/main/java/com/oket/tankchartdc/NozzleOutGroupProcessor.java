...
public class NozzleOutGroupProcessor implements Observer {
	 ...
	 * 液位轨迹没有匹配完成的最长时间.
	 */
	private static long NOZZLE_OUT_MAX_NOT_MATCHED_MILL;
	@Autowired
	private BackToTankService backToTankService;

	@Value(value = "${nozzleOutMaxNotMatchedDay}")
	public void setNozzleOutMaxNotMatchedMill(long nozzleOutMaxNotMatchedDay) {
		...
		for (BzNozzleOut bzNozzleOut : bzNozzleOuts) {
			...
			if (processed) {
				...
				if (timeDiff > NOZZLE_OUT_MAX_NOT_MATCHED_MILL) {
					...
					matchAlarm.setNozzleNo(Integer.parseInt(bzNozzleOut.getNozzleNo()));
...
//			数据库处理
								...
								if (!b.isEmpty()) {
									for (TransientOut transientOut : b) {
										...
										if (!transientOut.isOnFueling()) {
											...
											queryWrapper.eq("nozzle_no",systemAdditionNozzleOut.getNozzleNo());
											queryWrapper.le("start_time",systemAdditionNozzleOut.getStartTime());
											queryWrapper.orderByDesc("start_time");
											queryWrapper.last("limit 1");
											BzNozzleOut lastOut = nozzleOutDao.selectOne(queryWrapper);
											if(AirUtils.hv(lastOut)){
												systemAdditionNozzleOut.setPumpSum(DoubleUtils.add(lastOut.getPumpSum(),systemAdditionNozzleOut.getVolume(),2));
											}
											else {
	...
	private void mergeInventoryTraceInCache(List<AbstractLevelTrace> levelTraces) {
		...
		levelTrace.setLevelState(levelGap <= 0 ? LevelState.LEVEL_DESCENDING : LevelState.LEVEL_ASCENDING);
		//轨迹的更新
	...
	private synchronized void init() {
		try {
			log.info("NOZZLE_OUT_MAX_NOT_MATCHED_MILL:{}",NOZZLE_OUT_MAX_NOT_MATCHED_MILL);
