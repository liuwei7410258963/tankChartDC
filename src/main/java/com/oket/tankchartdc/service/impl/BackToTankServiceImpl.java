...
public class BackToTankServiceImpl extends ServiceImpl<BackToTankDAO, BackToTank> implements BackToTankService {
	...
	public void disableBacks(List<AbstractLevelTrace> levelTraces) {
		if (!levelTraces.isEmpty()) {
			final List<Long> traceIds = levelTraces.parallelStream().map(AbstractLevelTrace::getId).collect(toList());
			disable(traceIds);
		}
	...
	public void disableBackToTanks(List<DbInventoryTrace> levelTraces) {
		if (!levelTraces.isEmpty()) {
			final List<Long> traceIds = levelTraces.parallelStream().map(DbInventoryTrace::getId).collect(toList());
			disable(traceIds);
		}
	}
	...
	public List<BackToTank> upload(MultipartFile file) throws IOException {
		...
		if(!AirUtils.hv(exportLists)){
			return result;
		}
		exportLists.stream().sorted(Comparator.comparing(x -> x.getEndTime(), Comparator.nullsFirst(Date::compareTo))).collect(toList());
		QueryWrapper queryWrapper = new QueryWrapper();
		queryWrapper.ge("start_time",exportLists.get(0).getStartTime());
		queryWrapper.le("end_time",exportLists.get(exportLists.size()-1).getEndTime());
		List<BackToTank> dbLists = baseMapper.selectList(queryWrapper);
		if(AirUtils.hv(dbLists)) {
			//交集，被过滤的数据
			List<BackToTank> needFilterLists = dbLists.stream().filter(item -> exportLists.stream().map(e -> e.getStartTime())
					.collect(toList()).contains(item.getStartTime())).collect(toList());
		...
		this.saveBatch(exportLists.stream().filter(x->x.getFlag()==true).collect(toList()));
		saveConfirm(exportLists.stream().filter(x->x.getFlag()==true).collect(toList()));
		return result;
	}
