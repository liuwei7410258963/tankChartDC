...
public class CorrectSamplesServiceImpl extends ServiceImpl<CorrectSamplesDao, CorrectSamples> implements CorrectSamplesService {
    ...
    public void generateSamples(JSONObject jsonObject) {
        ...
        log.info("保存原有周期数据。"+lastInDbLists);
    }
    ...
    @Override
    public List<CorrectSamples> exportTable(JSONObject jsonObject) {
        List<Integer> queryIds = Arrays.asList(jsonObject.getString("delivery_id").split(",")).stream().map(Integer::new).collect(toList());
        jsonObject.put("tankNo",jsonObject.getIntValue("tank_no"));
        List<CorrectSamples> dbLists = getResultCorrectSamples(jsonObject, queryIds);
        return dbLists;
    }
