...
public class CorrectingServiceImpl implements CorrectingService {
    ...
    public List<CorrectingTableDTO> getResults(JSONObject jsonObject) {
        ...
        Map<Long, List<JSONObject>> map = dbList.stream().collect(Collectors.groupingBy(x->x.getLongValue("deliveryId")));
        for(List<JSONObject> m:map.values()){
            //如果第一笔是回罐，删掉
            removeFristBack(m);
        }
        dbList.clear();
        ...
        for(JSONObject x:dbList){
            if(x.get("group_id")==null){
                if(fristVolume==0.0){
                    ...
                    fristTime = x.getDate("trace_startime");
                    fristId = x.getLongValue("trace_startid");
                }
                ...
                if(fristVolume!=0.0) {
                    ...
                    x.put("trace_startime",fristTime);
                    x.put("trace_startid",fristId);
                    if(!AirUtils.hv(x.getDoubleValue("back_num"))){
                        x.put("back_num",0);
                    }
                    x.put("back_num", x.getDoubleValue("back_num")+backSum);
                    x.put("back_status",1);
                    backSum = 0.0;
                    fristVolume = 0.0;
                    fristLeave = 0.0;
     ...
     * 递归，如果第一笔是回罐，删掉
     */
    private void removeFristBack(List<JSONObject> dbList) {
        if(dbList.get(dbList.size()-1).get("group_id")==null){
            dbList.remove(dbList.size()-1);
            removeFristBack(dbList);
        }
        else{
            return;
        }
