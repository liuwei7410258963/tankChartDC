...
public class AbstractBalanceService implements BalanceService {
	 ...
	 */
	public final ConcurrentMap<Integer, TankSession> nozzle2TankSessions = new ConcurrentHashMap<>();

	@Override
	public void addObserver(Observer observer) {
		nozzleTemperatureObservable.addObserver(observer);
	}

	@Getter
	private final CommonObservable nozzleTemperatureObservable = new CommonObservable();
	...
	public void init() {
		try {
			...
			inventoryConsumer.removeErrorMap();
	 ...
	 * @param nozzleState
	 */
	private void notify(NozzleState nozzleState) {
		JSONObject jsonObject = new JSONObject();
		jsonObject.put("TYPE", "NOZZLE_STATE");
		jsonObject.put("nozzleNo", nozzleState.getNozzleNo());
		jsonObject.put("onFueling", nozzleState.isOnFueling());
		jsonObject.put("volume", nozzleState.getCurrentVolume());
		nozzleTemperatureObservable.setChanged();
		nozzleTemperatureObservable.notifyObservers(jsonObject);
	}
