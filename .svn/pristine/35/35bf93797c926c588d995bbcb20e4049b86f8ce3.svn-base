...
public class AbnormalServiceImpl extends ServiceImpl<AbnormalDao, AbnormalEntity> implements AbnormalService {
    ...
    public IPage<AbnormalEntity> query(JSONObject req, boolean isPage) {
        ...
        //查询泵码数异常
            ...
            wrapper.eq("tank_no",req.getIntValue("tankNo"));
            if (req.getString("startTime") != null && req.getString("endTime") != null) {
    ...
    public void update(JSONObject jsonObject){
        ...
        queryWrapper.eq("id",jsonObject.getIntValue("id"));
        AbnormalEntity abnormalEntity = baseMapper.selectOne(queryWrapper);
        abnormalEntity.setStatus(jsonObject.getBoolean("status"));
        baseMapper.updateById(abnormalEntity);
    }
}
